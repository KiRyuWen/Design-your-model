...
void OpenGLPanel_Example::MyForm::Load_Texture_Btn_Click(System::Object^ sender, System::EventArgs^ e)
{
	...
	if (openFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK)
	{
		...
		pictureBox->Size = System::Drawing::Size(128, 128);
		...
		//dataGridView1->Rows->Add(Image::FromFile(gcnew System::String(FileName)));
...
void OpenGLPanel_Example::MyForm::CreatePictureBox(String^ FileName)
{
	...
	this->flowLayoutPanel1->Controls->Add(pictureBox);
	textureRecord[fileName] = textureRecord.size();
}
...
System::Void OpenGLPanel_Example::MyForm::hkoglPanelControl2_Paint(System::Object^ sender, System::Windows::Forms::PaintEventArgs^ e)
{
	...
	{
		////render to post effect framebuffer
		//this->postEffectFbo2->UseFBO();
		//glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
		//glClearColor(0, 0, 0, 0);
		//glViewport(0, 0, this->hkoglPanelControl2->Size.Width, this->hkoglPanelControl2->Size.Height);
		//glUseProgram(paramGL2->postEffectInversionShader);
		//this->postEffectFbo2->UseQuadVAO();
		//glUniform1i(glGetUniformLocation(this->paramGL2->postEffectInversionShader, "screenTexture"), 0);
		...
		//this->postEffectFbo2->UseFBOTexture();
		glBindTexture(GL_TEXTURE_2D, model2->textures[currentTextureID - 1]->textureID);
...
{
	if (model->selectedFace.size() <= 0 || textureImages.size() == 0|| selectTextureImage==-1)
	{
		return;
	}
